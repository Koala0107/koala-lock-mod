...
public final class SmartphoneScreen extends Screen {
    ...
    protected void init() {
        ...
        nameField = addDrawableChild(new TextFieldWidget(textRenderer,
                innerX, panelY + 211, 94, 18, Text.translatable("screen.korime_scene.smartphone.name")));
        ...
        timeField = addDrawableChild(new TextFieldWidget(textRenderer,
                innerX + 98, panelY + 211, 58, 18, Text.translatable("screen.korime_scene.smartphone.time")));
        ...
        addButton = addDrawableChild(ButtonWidget.builder(
                        Text.translatable("screen.korime_scene.smartphone.add"), button -> addRecord())
                .dimensions(innerX, panelY + 232, actionWidth, 16)
                .build());
        applyButton = addDrawableChild(ButtonWidget.builder(
                        Text.translatable("screen.korime_scene.smartphone.apply"), button -> applyRecord())
                .dimensions(innerX + actionWidth + actionGap, panelY + 232, actionWidth, 16)
                .build());
        deleteButton = addDrawableChild(ButtonWidget.builder(
                        Text.translatable("screen.korime_scene.smartphone.delete"), button -> deleteRecord())
    ...
    public void render(DrawContext context, int mouseX, int mouseY, float delta) {
        ...
        Text status = Text.translatable("screen.korime_scene.smartphone.page",
    ...
    private void drawRecordList(DrawContext context, int mouseX, int mouseY) {
        ...
        for (int row = 0; row < PAGE_SIZE; row++) {
            ...
            String name = record.name().isBlank()
                    ? Text.translatable("screen.korime_scene.smartphone.unnamed").getString()
                    : record.name();
    ...
    private String callSecondaryText(String detail) {
        if (detail.isBlank()) {
            return "☎  " + Text.translatable("screen.korime_scene.smartphone.calls").getString();
        }
