...
public final class SmartphoneScreenV2 extends Screen {
    ...
    private SmartphoneScreenV2(Hand hand, SmartphoneData data, boolean allowEditing) {
        super(Text.translatable("screen.korime_scene.smartphone"));
        this.hand = hand;
    ...
    protected void init() {
        ...
        callsTab = addDrawableChild(ButtonWidget.builder(Text.translatable("screen.korime_scene.smartphone.calls"), b -> switchMode(true))
                .dimensions(innerX, panelY + 31, (innerWidth - 4) / 2, 17).build());
        messagesTab = addDrawableChild(ButtonWidget.builder(Text.translatable("screen.korime_scene.smartphone.messages"), b -> switchMode(false))
        ...
        if (editable) {
            ...
            nameField = addDrawableChild(new TextFieldWidget(textRenderer, innerX + 48, fieldsY, 62, 18,
                    Text.translatable("screen.korime_scene.smartphone.name")));
            nameField.setMaxLength(SmartphoneData.MAX_NAME_LENGTH);
            nameField.setPlaceholder(Text.translatable("screen.korime_scene.smartphone.name"));
            timeField = addDrawableChild(new TextFieldWidget(textRenderer, innerX + 113, fieldsY, 47, 18,
                    Text.translatable("screen.korime_scene.smartphone.time")));
            timeField.setMaxLength(SmartphoneData.MAX_TIME_LENGTH);
            timeField.setPlaceholder(Text.translatable("screen.korime_scene.smartphone.time"));
            detailField = addDrawableChild(new TextFieldWidget(textRenderer, innerX + 163, fieldsY, innerWidth - 163, 18,
                    Text.translatable("screen.korime_scene.smartphone.detail")));
            detailField.setMaxLength(SmartphoneData.MAX_DETAIL_LENGTH);
            detailField.setPlaceholder(Text.translatable("screen.korime_scene.smartphone.detail"));

            int buttonY = panelY + 180;
            int gap = 3;
            int buttonWidth = (innerWidth - gap * 3) / 4;
            addButton = addDrawableChild(ButtonWidget.builder(Text.translatable("screen.korime_scene.smartphone.add"), b -> addRecord())
                    .dimensions(innerX, buttonY, buttonWidth, 16).build());
            applyButton = addDrawableChild(ButtonWidget.builder(Text.translatable("screen.korime_scene.smartphone.apply"), b -> applyRecord())
                    .dimensions(innerX + buttonWidth + gap, buttonY, buttonWidth, 16).build());
            deleteButton = addDrawableChild(ButtonWidget.builder(Text.translatable("screen.korime_scene.smartphone.delete"), b -> deleteRecord())
                    .dimensions(innerX + (buttonWidth + gap) * 2, buttonY, buttonWidth, 16).build());
            finalizeButton = addDrawableChild(ButtonWidget.builder(Text.translatable("screen.korime_scene.smartphone.finalize"), b -> finalizePhone())
    ...
    public void render(DrawContext context, int mouseX, int mouseY, float delta) {
        ...
        context.drawCenteredTextWithShadow(textRenderer,
                Text.translatable("screen.korime_scene.smartphone.page", page + 1, totalPages, records.size()),
    ...
    private void drawPhoneChrome(DrawContext context) {
        ...
        context.drawTextWithShadow(textRenderer,
                showingCalls ? Text.translatable("screen.korime_scene.smartphone.calls") : Text.translatable("screen.korime_scene.smartphone.messages"),
    ...
    private void drawRecordList(DrawContext context, int mouseX, int mouseY) {
        ...
        for (int row = 0; row < PAGE_SIZE; row++) {
            ...
            String name = record.name().isBlank() ? Text.translatable("screen.korime_scene.smartphone.unnamed").getString() : record.name();
    ...
    private Text directionText(boolean outgoing) {
        if (showingCalls) return Text.translatable(outgoing ? "screen.korime_scene.smartphone.call_outgoing" : "screen.korime_scene.smartphone.call_incoming");
        return Text.translatable(outgoing ? "screen.korime_scene.smartphone.message_outgoing" : "screen.korime_scene.smartphone.message_incoming");
    }
    ...
    private void finalizePhone() {
        ...
        commitFields();
        String title = savedTitle.isBlank() ? Text.translatable("screen.korime_scene.smartphone.save.default_name").getString() : savedTitle;
        String subtitle = savedSubtitle.isBlank() ? Text.translatable("screen.korime_scene.smartphone.save.default_subtitle").getString() : savedSubtitle;
