...
public final class LockState extends PersistentState {
    ...
    public static LockState get(ServerWorld world) { return world.getPersistentStateManager().getOrCreate(TYPE, STORAGE_KEY); }

    public Optional<LockEntry> get(BlockPos pos) {
        LockEntry direct = locks.get(pos.asLong());
        if (direct != null) return Optional.of(direct);

        // Old worlds/builds could leave only one half of a double chest indexed.
        // Fallback only when exactly one adjacent chest lock exists, avoiding unrelated neighboring locks.
        LockEntry candidate = null;
        for (Direction direction : Direction.Type.HORIZONTAL) {
            LockEntry nearby = locks.get(pos.offset(direction).asLong());
            if (nearby == null || !nearby.blockId().contains("chest")) continue;
            if (candidate != null && !candidate.lockId().equals(nearby.lockId())) return Optional.empty();
            candidate = nearby;
        }
        return Optional.ofNullable(candidate);
    }

    public Collection<Map.Entry<Long, LockEntry>> entries() { return locks.entrySet(); }
    public void put(BlockPos pos, LockEntry entry) { locks.put(pos.asLong(), entry); markDirty(); }
    ...
    private static LockState fromNbt(NbtCompound nbt) {
        ...
        for (int index = 0; index < list.size(); index++) {
            ...
            try {
                ...
                String type = value.contains("Type", NbtElement.STRING_TYPE) ? value.getString("Type") : CrouchLockMod.KEY_LOCK;
                String credential = value.contains("Credential", NbtElement.STRING_TYPE) ? value.getString("Credential") : value.getString("Key");
                ...
                if (!credential.isBlank()) state.locks.put(pos, new LockEntry(type, credential, lockId, ownerId, blockId));
